...
- [Adaptive Import](storage-cosmosdb-adaptive-import.md) — how `importBulk` adapts concurrency to RU-constrained tiers (control loop, throttle detection, circuit breaker).
- [Gremlin Compatibility & Performance Notes](cosmosdb-gremlin-compatibility.md) — what we've verified works (and doesn't) in CosmosDB's Gremlin subset. **Read this before changing emitted Gremlin** in the compiler or any query module.
