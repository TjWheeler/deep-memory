...
| `Module` | A non-test source file — the file-level layer beneath the package graph. Carries a `package` property and a `throwsRawError` flag. | every non-test `packages/*/src/**/*.ts` |
...
| `Doc ─DOCUMENTS→ code` | a markdown link to a source file (precise) |
| `Doc ─MENTIONS→ ProviderContract \| ProviderImpl` | a symbol name in prose (broader) |
| `Doc ─DESCRIBES→ Package` | a package README documents its own package (the reliable doc↔package link) |
| `Test ─COVERS→ ProviderImpl \| ProviderContract \| McpTool` | the test imports the symbol; `role` separates the `subject` (matches the test filename) from a `fixture` — so "which tools have no test" is a query |
...
- **Deterministic extraction, no LLM.** Each layer is mined straight from the source: `package.json`
  ...
  for docs, import declarations for tests (resolved against providers *and* tool classes), per-file
  import/export specifiers for the module graph, `throw new` sites for THROWS, and `extends *Error`
...
| [scripts/code-graph/extract-tests.ts](../scripts/code-graph/extract-tests.ts) | ts-morph: the symbols each co-located test imports (COVERS). |
...
| [scripts/code-graph/rebuild.ts](../scripts/code-graph/rebuild.ts) | The orchestrator — extract → connect Neo4j → reconcile (delta) → verify. Entry point of `pnpm code-graph:rebuild`. |
...
- **`DOCUMENTS` is sparse.** Docs rarely link directly to a `.ts` file, so the doc→code routing value
  lives mostly in the broader `MENTIONS` edges (and, for packages, the reliable `DESCRIBES` edge).
