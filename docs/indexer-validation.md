...
- **Disable thinking mode for reasoning-capable local workers.** The full-validation worker's final turn asks the model to synthesize a verdict for every item in the batch as one JSON object — a harder, more open-ended task than a single tool call. A reasoning model can spend its entire `maxTokens` budget on internal chain-of-thought before writing any of that JSON, producing `finish_reason: "length"` with **zero characters of actual content** — every item in the batch then falls back to `unverifiable` ("worker did not return a result"), and raising `maxTokens` further doesn't reliably fix it because reasoning length varies per batch rather than scaling predictably with a bigger budget. If the model supports it, set `extraBodyParams: { "chat_template_kwargs": { "enable_thinking": false } }` (the exact kwarg name is model/template-specific — check the endpoint's chat template) on the full-validation worker, the same way the extraction workers above do it. Diagnose this failure mode from the per-batch logs (`logDir`): a `*_turn-N-text.json` entry with `finishReason: "length"` and `contentLength: 0`, paired with `outputTokens` equal to the configured `maxTokens`, is the signature.
