-- ============================================================
-- @utaba/deep-memory SQL Server schema v1
-- ============================================================

-- Schema version tracking
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'dm_meta' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
  CREATE TABLE [dbo].[dm_meta] (
    [key]   NVARCHAR(100)  NOT NULL,
    [value] NVARCHAR(MAX)  NOT NULL,
    CONSTRAINT [pk_dm_meta] PRIMARY KEY ([key])
  );
  INSERT INTO [dbo].[dm_meta] ([key], [value])
  VALUES ('schema_version', '1');
END;

-- Repositories
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'dm_repositories' AND schema_id = SCHEMA_ID('dbo'))
CREATE TABLE [dbo].[dm_repositories] (
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'dm_vocabularies' AND schema_id = SCHEMA_ID('dbo'))
CREATE TABLE [dbo].[dm_vocabularies] (
  ...
  CONSTRAINT [fk_dm_vocabularies_repositories]
    FOREIGN KEY ([repository_id]) REFERENCES [dbo].[dm_repositories]([repository_id])
    ON DELETE CASCADE
);

-- Vocabulary change log
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'dm_vocabulary_change_log' AND schema_id = SCHEMA_ID('dbo'))
CREATE TABLE [dbo].[dm_vocabulary_change_log] (
...
CREATE NONCLUSTERED INDEX [ix_dm_vocabulary_change_log_proposed_at]
  ON [dbo].[dm_vocabulary_change_log] ([repository_id], [proposed_at] DESC);

-- Entities
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'dm_entities' AND schema_id = SCHEMA_ID('dbo'))
CREATE TABLE [dbo].[dm_entities] (
...
IF NOT EXISTS (
  ...
  WHERE t.name = 'dm_entities' AND t.schema_id = SCHEMA_ID('dbo')
)
CREATE FULLTEXT INDEX ON [dbo].[dm_entities] (
  [label]      LANGUAGE 1033,
...
-- Relationships
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'dm_relationships' AND schema_id = SCHEMA_ID('dbo'))
CREATE TABLE [dbo].[dm_relationships] (
  ...
  CONSTRAINT [fk_dm_relationships_source]
    FOREIGN KEY ([repository_id], [source_entity_id]) REFERENCES [dbo].[dm_entities]([repository_id], [entity_id]),
  CONSTRAINT [fk_dm_relationships_target]
    FOREIGN KEY ([repository_id], [target_entity_id]) REFERENCES [dbo].[dm_entities]([repository_id], [entity_id])
);

CREATE NONCLUSTERED INDEX [ix_dm_relationships_source]
  ON [dbo].[dm_relationships] ([repository_id], [source_entity_id], [relationship_type])
  INCLUDE ([target_entity_id], [bidirectional]);
CREATE NONCLUSTERED INDEX [ix_dm_relationships_target]
  ON [dbo].[dm_relationships] ([repository_id], [target_entity_id], [relationship_type])
  INCLUDE ([source_entity_id], [bidirectional]);
CREATE NONCLUSTERED INDEX [ix_dm_relationships_type]
  ON [dbo].[dm_relationships] ([repository_id], [relationship_type]);

-- Relationship provenance indexes
CREATE NONCLUSTERED INDEX [ix_dm_relationships_created_conversation]
  ON [dbo].[dm_relationships] ([repository_id], [created_in_conversation]);

CREATE NONCLUSTERED INDEX [ix_dm_relationships_modified_at]
  ON [dbo].[dm_relationships] ([repository_id], [modified_at] DESC);
...
IF NOT EXISTS (SELECT 1 FROM sys.types WHERE name = 'dm_id_list' AND is_table_type = 1)
  CREATE TYPE [dbo].[dm_id_list] AS TABLE ([id] NVARCHAR(300) NOT NULL);

GO

CREATE OR ALTER PROCEDURE [dbo].[dm_search_entities]
...
BEGIN
  ...
  ;WITH ft_results AS (
    ...
    FROM (
      SELECT [KEY], [RANK]
      FROM FREETEXTTABLE([dbo].[dm_entities], ([label], [summary], [data], [properties]), @Query)
      WHERE @UseContains = 0
      UNION ALL
      SELECT [KEY], [RANK]
      FROM CONTAINSTABLE([dbo].[dm_entities], ([label], [summary], [data], [properties]), @Query)
      WHERE @UseContains = 1
    ) AS ft
    INNER JOIN [dbo].[dm_entities] e ON e.[ft_key] = ft.[KEY]
