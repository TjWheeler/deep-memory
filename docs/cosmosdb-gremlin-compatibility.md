...
| `.sideEffect(__.V().has('repositoryId', otherPartition).hasId(x).property(...))` | **Single-submit cross-partition write.** A `sideEffect` step starting with `__.V().has('repositoryId', <other>)` lets one Gremlin submit mutate a vertex in a different partition than the parent traverser. Live-validated 2026-05-26 — the sentinel pattern in `createRepository` uses this to update `_repository_index` in the `_index` partition atomically with the new repo vertex's addV. **Caveat:** "atomicity" is per-submit, not transactional — if the sub-step fails, the parent may have already succeeded. The probe also confirmed the reverse direction: drop a vertex in partition A while sideEffect-updating a sentinel in partition B in one submit. |
...
### `hasId(x)` alone does NOT auto-route — partition predicate is mandatory

A separate gotcha worth calling out explicitly because the surface intuition is wrong: even when a vertex's synthetic id encodes its repository (e.g. `repo:<uuid>`, `vocab:<uuid>`), Cosmos Gremlin does **not** infer the partition from the id. The container's partition key is `/repositoryId` — a *property*, not the system id — and `hasId(x)` is post-routing in Cosmos. Without an explicit `has('repositoryId', rid)` predicate, the engine fans the lookup out to every physical partition and asks each one "do you have this id?".

The 2026-05-26 audit caught seven system-vertex queries (`_repository`, `_vocabulary`) anchored only by `hasId(vid)` — all fanning out. The fix is mechanical: insert `.has('repositoryId', rid)` before `.hasId(vid)` and bind `rid` alongside `vid`. There's no scenario where the partition predicate adds cost — it's pure savings. Captured as memory `feedback-cosmos-partition-key-required.md`.
...
| Live shape probes — cross-partition sideEffect-update (2026-05-26) | The sentinel pattern from [local-tests/phase11-shape-probe.mjs](../local-tests/phase11-shape-probe.mjs): (1) a sentinel vertex pinned in a fixed `_index` partition (id=`_repository_index`, repositoryId=`_index`) can be created, read, and `coalesce(unfold().property(...), addV(...))`-upserted exactly like any other vertex. (2) `g.addV('_repository').property('repositoryId', newRid)...sideEffect(__.V().has('repositoryId', '_index').hasId('_repository_index').property('repositoryIds', updatedJson)).id()` works in a SINGLE submit — the addV happens in the new repo's partition and the sentinel update fires as a sideEffect in the `_index` partition. (3) Same shape works for delete: `g.V().has('repositoryId', rid).hasId(repoVid).sideEffect(__.V().has('repositoryId', '_index').hasId('_repository_index').property('repositoryIds', updatedJson)).drop()` mutates the sentinel during the drop of the parent vertex. (4) Per-submit "atomicity" is not the same as transactionality — if the sub-step fails after the parent succeeds, the two partitions can drift. listRepositories drops sentinel entries that no longer have a backing vertex defensively, which papers over the rare partial-failure window. |
